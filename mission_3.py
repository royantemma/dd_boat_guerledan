...
def lissajou(t):
    return A*np.sin((t-t0-delta_i)/T*2*np.pi)+a[0], A*np.sin(2*(t-t0-delta_i)/T*2*np.pi)+a[1]
...
def direction():
    ...
    #direction
    d = (e/np.linalg.norm)*np.tanh(np.linalg.norm(e)/5) + pd_p
